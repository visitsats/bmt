              ...
              <form role="form" class="wrap mt25" >
			   ...
			   <?php
				...
				Enter Number Of Tables
				 <input type="text" placeholder="No of Tables" class="form-control"  maxlength="2"  name="no_of_tables" required id="no_of_tables" value="<?php echo $floor_info[0]['no_of_tables'];?>">
                </div>
                <div class="form-group col-md-6">
				Enter Number of Rows
				 <input type="text" placeholder="No of rows" class="form-control"   maxlength="2" name="no_of_rows" required id="no_of_rows" value="<?php echo $floor_info[0]['floor_rows'];?>"> <br/>
                </div>
                <div class="form-group col-md-6">
				Enter Number of Columns
				 <input type="text" placeholder="No of Columns" maxlength="2" class="form-control no_of_columns"   name="no_of_columns" required id="no_of_columns" value="<?php echo $floor_info[0]['floor_columns'];?>">
                </div>
              </form>
            </div>
          </div>
  ...
  <div class="floor-layout">
	...
	//echo '<pre>';print_r($floor_array);
		...
		$output.= '<td><div class="'.$class.'" id="change_color_'.$k.'"><span id="change_capacity_'.$k.'" >'.$table_no.'</span><div class="hover"><a href="javascript:void(0)" id="click_model_'.$k.'" title="'.$x.'" onclick="click_model('.$k.')" data-toggle="modal" data-target="#myModal_'.$k.'"  class="btn btn-sm btn-default">Edit</a><a href="javascript:void(0)"  title="'.$x.'" onclick="delete_table('.$k.')"  class="btn btn-sm btn-default btnresize">Delete</a></div></div><input type="text" style="display:none" id="click_input_'.$k.'" value="" /><input type="text" style="display:none" value="'.$x.''.$n.'" name="serialno" /></td>
		';
		$i++;
		$j++;
        $k++;
  ...
  <div class="form-group col-md-9"><button  class="btn btn-success pull-right icon-btn" type="button" onclick="save_tables()"> <i class="fa fa-plus-circle"></i> Update Floor</button> </div>
    <div id="load_popup_data">  <?php
$i=1;
foreach($floor_info as $floor_info1)
{
?>

  <div class="modal fade table-selection" id="myModal_<?php echo $i;?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
...
?></div>
		<script type="text/javascript">
			...
			{				
				...
				$('#change_color_'+id).removeClass('tb rectangle six green');
