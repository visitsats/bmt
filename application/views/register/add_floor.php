            ...
            <div class="panelone  plr10">
              ...
              <form role="form" class="wrap mt10" >
			   ...
			   <?php
				...
				<label class="form-lable "> Floor Name<span class="star">*</span></label>	
				 <input type="text" placeholder="" class="form-control" id="floor_no"  name="floor_no" maxlength="25" required>
                </div>
                <div class="form-group col-md-6">
  ...
  <a href="<?php echo base_url().'bookmyt/floors/'; ?>" class="btn btn-success pull-right" style="margin-left:5px">Cancel</a><button  class="btn btn-success pull-right icon-btn" type="button" onclick="save_tables()"> <i class="fa fa-plus-circle"></i> Add Floor</button> </div>
    <div id="load_popup_data"></div>
		...
		$('#no_of_tables,#no_of_rows,#no_of_columns').keydown(function(event) 
		   ...
		   $.ajax({
			type :	"POST",
			url	 :	"<?php echo base_url();?>bookmyt/addfloor",
			...
			'no_of_rows':$("#no_of_rows").val(),'no_of_columns':$("#no_of_columns").val(),'table_type':table_type,'business_id':business_id},
			success : function(data)
			{
				if($.trim(data)==1)
				{
			...
			function delete_table(id)
			{
				...
				if(confirm("Are you sure you want to delete the table")){
					...
					{
						$('#click_model_'+id).text('<i class="fa fa-plus" aria-hidden="true">');
					}
				}
			}
