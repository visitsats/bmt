...
class Business extends REST_Controller
{
	...
	*/
	...
	function add_business_customer_post()
    {
        ...
        $business_name  = $this->post('business_name');
        $business_email = $this->post('email_phone');
		$business_phone = $this->post('phone_number');
        $location = $this->post('city');
		$sub_type=$this->post('sub_type');
		$end_date=$this->post('end_date');
		$start_date=$this->post('start_date');
		$zip=$this->post('zipcode');
		...
		$business_type=$this->post('bus_type');
		$no_of_users=(($this->post('no_of_users') !='')?$this->post('no_of_users'):0);
		$country = $this->post('country');
		$state = $this->post('state');
			...
			$time_zone=$this->post("t_zone");
				...
				$created_date = date("Y-m-d H:i:s");
					...
					$input_array  = array(
						...
						'i_agress'	=> $this->input->post('i_agree')
					);
					$contact_array  = array(
						
						'name' => $business_name,
						'email' => $business_email,
						'contact_type' => "Request For Demo During Registration",
				...
				if($want_demo){
					$contact_list=$this->users_model->KM_save(array(
					'class' => "contact_list",
					'insert' => $contact_array,
					'return_id' => true
				));
				}
				if ($userid) 
	...
	function edit_branch_post()
    {
		...
		$address       = $this->input->post('address');
		$email          = $this->input->post('email');
		$business_typeid = ($this->input->post('business_types')!="")?$this->input->post('business_types'):'1';
		$time_zone    = $this->input->post('time_zone');
		$business_id=$this->input->post('business_id');
	...
	*/ 
	...
	public function add_user_post()
	{
		...
		//if(!is_numeric($this->input->post('email_phn'))){
			...
			$insert_array=array(
							'username' => $this->input->post('username'),
							'password' => $this->input->post('rand_pwd'),
							'phone_no' => $this->input->post('email_phn'),
							"user_type_id" => $user_type,
							'business_id' => $this->input->post('branch'),
							'relationship_id' => $relationship_id
						);
		...
		$sql="select u.username,u.password,u.email,u.phone_no,r.role_name,b.business_name from user_details u
				join role_permissions r on r.role_id=u.user_type_id
				join business_entity b on b.business_id=u.business_id 
	...
	public function update_user_post()
	{
		...
		}else if(is_numeric($this->input->post('email_phn'))){
			$data = array(
				'username' => $this->input->post('username'),
				'phone_no' => $this->input->post('email_phn'),
				"user_type_id" => $user_type,
